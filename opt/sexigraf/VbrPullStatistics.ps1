...
if ($($VbrJobsStates.data)) {
        ...
        if (Test-Path /mnt/wfs/inventory/ViVmInventory.csv) {
            ...
            if ($ViVmInventory) {
                ...
                if ($VbrVmInventoryTable.Values) {
                    ...
                    $VbrVmInventories = Get-ChildItem "/mnt/wfs/inventory/VbrVmInventory.*.csv"

                    if ($VbrVmInventories) {
                        SexiLogger "[INFO] Rotating VbrVmInventory.*.csv files ..."
                        $ExtraCsvFiles = Compare-Object  $VbrVmInventories  $($VbrVmInventories|Sort-Object LastWriteTime | Select-Object -Last 10) -property FullName | ?{$_.SideIndicator -eq "<="}
                        If ($ExtraCsvFiles) {
                            try {
                    ...
                    try {
                        ...
                        if ($ExecStart.DayOfWeek -match "Monday" -and !$($VbrVmInventories|?{$_.LastWriteTime -gt $ExecStart.AddDays(-1)})) {
    ...
    try {
        ...
        $VbrEndedSessions = Invoke-RestMethod -SkipHttpErrorCheck -SkipCertificateCheck -Method GET -Uri $("https://" + $server + ":9419/api/v1/sessions?typeFilter=Job&endedAfterFilter=" + $(($ExecStart.AddMinutes(-5)).ToString("o"))) -Headers $VbrAuthHeaders
    } catch {
